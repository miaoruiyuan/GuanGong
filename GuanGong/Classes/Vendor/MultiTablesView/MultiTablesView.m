...
#pragma mark Default Headers and Footers Heights
- (CGFloat)defaultHeightForRowAtIndexPath:(NSIndexPath *)indexPath atLevel:(NSInteger)level{
    return [self tableViewAtIndex:level].rowHeight;
}
- (CGFloat)defaultHeightForHeaderInSection:(NSInteger)section atLevel:(NSInteger)level {
...
- (nullable NSArray<NSString *> *)sectionIndexTitlesForTableView:(UITableView *)tableView{
    NSArray *indexTitles = [NSArray array];
    if ([self.dataSource respondsToSelector:@selector(multiTablesView:sectionIndexTitlesAtLevel:)]) {
        indexTitles = [self.dataSource multiTablesView:self sectionIndexTitlesAtLevel:[self indexOfTableView:tableView]];
    }
    return indexTitles;
}
...
- (CGFloat )tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([self.delegate respondsToSelector:@selector(multiTablesView:level:heightForRowAtIndexPath:)]) {
        
        heightForRow = [self.delegate multiTablesView:self level:[self indexOfTableView:tableView] heightForRowAtIndexPath:indexPath];
    }else {
        heightForRow = [self defaultHeightForRowAtIndexPath:indexPath atLevel:[self indexOfTableView:tableView]];
    }
    return heightForRow;
...
- (NSInteger)tableView:(UITableView *)tableView sectionForSectionIndexTitle:(NSString *)title atIndex:(NSInteger)index{
    ...
    if ([self.delegate respondsToSelector:@selector(multiTablesView:level:sectionForSectionIndexTitle:atIndex:)]) {
        endIndex = [self.delegate multiTablesView:self level:[self indexOfTableView:tableView] sectionForSectionIndexTitle:title atIndex:index];
    }
