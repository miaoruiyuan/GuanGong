...
- (void)setPhoto:(PYPhoto *)photo
{
    ...
    [self setImageWithURL:url placeholder:placeholdeerImage options:YYWebImageOptionShowNetworkActivity progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        @strongify(self);
        if (self.isBig) {
            // 获取图片链接
            NSString *imageUrl = [[self imageURL] absoluteString];
        ...
        self.progressView.hidden = YES;
        if (image && ([[url absoluteString] isEqualToString:self.photo.thumbnail_pic] ||
                      [[url absoluteString] isEqualToString:self.photo.original_pic])) { // 图片为当前PYPhotoView的图片并且不是占位图（占位图 image会为null）
            // 允许手势
            [self addGestureRecognizers];
            // 记录原始大小
