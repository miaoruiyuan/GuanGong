...
@interface GGScanCodeViewController ()<AVCaptureMetadataOutputObjectsDelegate>
...
@implementation GGScanCodeViewController
...
- (void)configUI{
    ...
    CGFloat padding = (kScreenWidth - width)/2;
    CGRect scanRect = CGRectMake(padding, kScreenHeight/4, width, width);
...
- (void)analyseResult:(AVMetadataMachineReadableCodeObject *)result{
    ...
    //交给 block 处理
    if (_scanResultBlock) {
        NSString *mobile = [[resultStr componentsSeparatedByString:@"params="] lastObject];
        _scanResultBlock(self, mobile);
    }
    [self pop];
}
