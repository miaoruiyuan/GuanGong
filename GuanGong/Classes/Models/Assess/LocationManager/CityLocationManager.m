...
single_implementation(CityLocationManager);
...
-(CLLocationManager *)locationM
{
    if (!_locationM) {
        ...
        if (isIOS(8.0)) {
            ...
            else if ([whenInUseStr length] > 0)
            {
                ...
                if (![backModes containsObject:@"location"]) {
                    DLog(@"当前授权模式是前台定位授权, 如果想要在后台获取位置, 需要勾选后台模式location updates");
            ...
            {
                DLog(@"如果在iOS8.0之后获取用户位置, 必须主动填写info.plist文件中的key NSLocationAlwaysUsageDescription 或者 NSLocationWhenInUseUsageDescription");
            }
        ...
        }else // ios8.0之前
        {
            ...
            NSArray *backModes = infoDic[@"UIBackgroundModes"];
            if (![backModes containsObject:@"location"]) {
                DLog(@"当前授权模式, 如果想要在后台获取位置, 需要勾选后台模式location updates");
            }
